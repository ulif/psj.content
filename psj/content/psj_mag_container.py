...
#  MA 02111-1307 USA.
#
"""Dexterity base magazine container type for PSJ.
...
class IPSJMagazineContainer(model.Schema):
    """A PSJ base.
    """
    ...
    psj_urn = schema.TextLine(
        title=_(u'URN'),
        description=_(u'Uniform Resource Name'),
        required=True,
        )


class PSJMagazineContainer(Container):
    """A PSJ Magazine Container.
    """
    grok.implements(IPSJMagazineContainer)
    ...
    psj_title = FieldProperty(IPSJMagazineContainer["psj_title"])
    psj_subtitle = FieldProperty(IPSJMagazineContainer["psj_subtitle"])
    psj_publication_year = FieldProperty(
        IPSJMagazineContainer["psj_publication_year"])
    psj_urn = FieldProperty(IPSJMagazineContainer["psj_urn"])
